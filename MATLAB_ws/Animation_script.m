...
if it > 1 
    if ~isempty(Sigma_tmp)
        lgd = legend([Est_PDF_plot, RealDef_plot, trayectoria_plot,...
               ...
               {"PDF Estimation, $\hat{\Phi}(\textit{\textbf{x}})$",...
               "Real defects",...
               "$\textit{\textbf{x}}(t)$",...
               "$\textit{\textbf{x}}(0)$",...
               "Sensor footprint",...
...
xlabel('Time [s]')
ylabel('$\varepsilon \left( \hat{\Phi}(\textit{\textbf{x}}), \textit{\textbf{x}}(t) \right) $')
grid on
xlim([0, t_f])
...
for eliminar_datos = 1:2 %Elimina la mitad de datos en cada ciclo
...
% *Save animation
myWriter = VideoWriter("Video2/Animacion_it_" + it, "Motion JPEG AVI");
