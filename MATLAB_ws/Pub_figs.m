...
layout1h.TileSpacing = 'tight';
layout1h.Padding = 'tight';
...
load("Results2/N150/2Def/output_6.mat");
...
folderPathBag = fullfile("/home", "gustavo-fuentevilla",...
                    "DefectsExp_UR", "Tests2", "N150", "2Def", "Test6",...
                    "synced_data_" + it + ".csv");
full_data = csvReading(folderPathBag);

planned_vel = [t, X_e_dot_reg(:,:,it)];

[Data_current, Preprocess_struct_tmp] = Preprocessing_tmp(full_data,...
                                            Thresholds,...
                                            planned_vel,...
                                            Estim_sol{it-1}.flag_ExplorationStage);

def_signal = Preprocess_struct_tmp.def_signal;
movingAvgAbsDerivative = Preprocess_struct_tmp.movingAvgAbsDerivative;
v_Norm = sqrt(sum(X_e_dot_reg(:,:,it).^2, 2));
v_Norm_spline = Preprocess_struct_tmp.v_Norm_spline;
abs_derivative_signal = Preprocess_struct_tmp.abs_derivative_signal;
